...
import type { PointageInsert } from "@/types/pointage";
...
export async function POST(request: Request) {
  try {
    ...
    // Si les variables d'environnement sont configurées, on applique la sécurité stricte
    if (companyLat !== 0 && companyLng !== 0) {
      if (!latitude || !longitude) {
        return NextResponse.json(
          { error: "Géolocalisation requise pour pointer." },
          { status: 403 }
        );
      }
      ...
      if (!isAllowed) {
        return NextResponse.json(
          { error: "Vous êtes hors du périmètre autorisé de l'entreprise." },
          { status: 403 }
        );
      }
    }
    ...
    // 1️ Vérifier l'authentification
    ...
    // 3️ Récupérer l'heure serveur (RPC)
    const { data: serverTime, error: rpcError } = await supabase.rpc(
      "get_server_time"
    );
    if (
      rpcError ||
      !serverTime ||
      !Array.isArray(serverTime) ||
      serverTime.length === 0
    ) {
      return NextResponse.json(
    ...
    // 4️ Vérifier si un pointage existe déjà aujourd'hui
    ...
    // 5️ Valider selon le type de pointage
    const validation = validerPointage(
      type,
      heureServeur,
      pointageExistantTyped
    );
    if (!validation.valid) {
      return NextResponse.json({ error: validation.error }, { status: 400 });
    }
    // 6️ Créer ou mettre à jour le pointage
    let result: Pointage | null = null;
    if (!pointageExistantTyped) {
      // Créer un nouveau pointage
      // Use Partial<PointageInsert> because not all fields are required initially,
      // but id_employe and date_pointage are mandatory in the DB.
      // However, PointageInsert type has id_employe as number, but here we might need to cast or ensure types match.
      // Let's use Partial<PointageInsert> and cast as needed or just use PointageInsert if we fill all required.
      // Actually, PointageInsert has optional fields.
      const insertData: Partial<PointageInsert> = {
        id_employe: parseInt(profilTyped.id_employe), // Ensure number if needed, or string if DB allows. PointageInsert says number.
        date_pointage: dateServeur,
        pointage_arrive: type === "arrive" ? heureServeur : null,
      };

      if (type === "arrive" && latitude && longitude) {
        insertData.lat_arrive = latitude;
        insertData.lon_arrive = longitude;
      }
      ...
      const updateData: Partial<PointageInsert> = {};
      if (type === "arrive") {
        updateData.pointage_arrive = heureServeur;
        if (latitude && longitude) {
          updateData.lat_arrive = latitude;
          updateData.lon_arrive = longitude;
        }
      }
      if (type === "pause") {
        updateData.pointage_pause = heureServeur;
        if (latitude && longitude) {
          updateData.lat_pause = latitude;
          updateData.lon_pause = longitude;
        }
      }
      if (type === "reprise") {
        updateData.pointage_reprise = heureServeur;
        if (latitude && longitude) {
          updateData.lat_reprise = latitude;
          updateData.lon_reprise = longitude;
        }
      }
      if (type === "depart") {
        updateData.pointage_depart = heureServeur;
        if (latitude && longitude) {
          updateData.lat_depart = latitude;
          updateData.lon_depart = longitude;
        }
      }
      const { data, error } = await supabase
        .from("pointage")
  ...
  switch (type) {
    ...
    case "pause":
      ...
      if (pointageExistant.pointage_depart) {
        return {
          valid: false,
          error: "Départ déjà pointé, impossible de pointer la pause",
        };
      }
      ...
      if (heureServeur < "12:30:00") {
        return {
          valid: false,
          error: "La pause est disponible à partir de 12h30",
        };
      }
      return { valid: true };
    ...
    case "depart":
      ...
      if (heureServeur < "16:00:00") {
        return {
          valid: false,
          error: "Le départ est disponible à partir de 16h00",
        };
      }
      return { valid: true };
