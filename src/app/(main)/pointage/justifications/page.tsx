...
export default function JustificationsPage() {
  ...
  const [filter, setFilter] = useState<"all" | "en_attente" | "traitees">(
    "en_attente"
  );
  const [typeFilter, setTypeFilter] = useState<"all" | "retard" | "absence">(
    "all"
  );
  ...
  const loadJustifications = useCallback(async () => {
    ...
    let filtered: PointageWithEmployee[] = (
      (data as PointageWithEmployee[]) || []
    ).filter((p) => p.justification_absence || p.justification_retard);
  ...
  const handleTraiter = async (
    ...
    try {
      ...
      alert(
        `✅ Justification ${decision === "justifiee" ? "approuvée" : "rejetée"}`
      );
      await loadJustifications();
    } catch (error) {
        ...
        <div className="flex space-x-4 mt-6">
          <div>
            ...
            <select
              value={filter}
              onChange={(e) =>
                setFilter(e.target.value as "all" | "en_attente" | "traitees")
              }
          ...
          <div>
            ...
            <select
              value={typeFilter}
              onChange={(e) =>
                setTypeFilter(e.target.value as "all" | "retard" | "absence")
              }
              className="px-4 py-2 border border-gray-300 rounded-md"
            >
                ...
                <div className="flex-1">
                  <div className="flex items-center space-x-3">
                    ...
                    <span className="text-sm text-gray-500">
                      {j.employe?.post_employe}
                    </span>
                  </div>
                  <p className="text-sm text-gray-600 mt-1">
                    📅 {formatDate(j.date_pointage)} •{" "}
                    {getTempsEcoule(j.created_at)}
                  </p>
                    ...
                    <div className="mt-4 p-4 bg-orange-50 border border-orange-200 rounded">
                      <div className="flex items-start justify-between mb-2">
                        <div>
                          <p className="font-semibold text-orange-800">
                            🕐 Retard de {j.retard_minutes} minutes
                          </p>
                          <p className="text-sm text-gray-600 mt-1">
                            Arrivée : {j.pointage_arrive}
                          </p>
                        </div>
                        <span
                      ...
                      {j.statut_justification_retard === "en_attente" && (
                        <div className="flex space-x-2 mt-3">
                          ...
                          <button
                            onClick={() =>
                              handleTraiter(j.id_pointage, "retard", "rejetee")
                            }
                    ...
                    <div className="mt-4 p-4 bg-red-50 border border-red-200 rounded">
                      ...
                      {j.statut_justification_absence === "en_attente" && (
                        <div className="flex space-x-2 mt-3">
                          ...
                          <button
                            onClick={() =>
                              handleTraiter(j.id_pointage, "absence", "rejetee")
                            }
                            className="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700 text-sm"
                          >
                            ✗ Rejeter
