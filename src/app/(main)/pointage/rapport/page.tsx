...
export default function RapportMensuelPage() {
  ...
  const [selectedPointage, setSelectedPointage] = useState<Pointage | null>(
    null
  );
  ...
  const handleSubmitJustification = async (
    ...
    try {
      ...
      alert(
        "✅ Justification envoyée avec succès. Elle sera examinée par un administrateur."
      );
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
          <div className="flex-1">
            ...
            <p className="text-xs sm:text-sm text-gray-500 mt-2">
              Horaires : 8h30 - 17h30 (8h effectif)
            </p>
          </div>
...
function StatCard({
  ...
  return (
    <div
      className={`${colorClass} rounded-lg p-4 sm:p-5 transition-transform hover:scale-105`}
    >
      <p className="text-xs sm:text-sm font-medium text-gray-700">{label}</p>
      <p className={`text-2xl sm:text-3xl font-bold ${valueClass} mt-2`}>
        {value}
      </p>
