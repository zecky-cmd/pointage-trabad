...
export default function JoursFeriesPage() {
  ...
  const loadJoursFeries = async (): Promise<void> => {
    const { data } = await supabase.from("jours_feries").select("*").order("date_ferie", { ascending: true })
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      if (editingId) {
        // Modification
        const { error } = await supabase.from("jours_feries").update(formData).eq("id_ferie", editingId)

        if (error) throw error
        alert("✅ Jour férié modifié avec succès")
      } else {
        // Création
        const { error } = await supabase.from("jours_feries").insert([formData])
  ...
  const handleDelete = async (id: number): Promise<void> => {
    ...
    try {
      const { error } = await supabase.from("jours_feries").delete().eq("id_ferie", id)
      ...
      <main className="max-w-5xl mx-auto py-6 sm:px-6 lg:px-8">
        <div className="bg-white rounded-lg shadow p-6 mb-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 mt-1">Gérer les jours fériés pour le calcul des pointages</p>
            </div>
        ...
        {joursFeries.length === 0 && (
          <div className="bg-white rounded-lg shadow p-12 text-center">
            ...
            <button onClick={() => openModal()} className="px-6 py-3 bg-blue-600 text-white rounded hover:bg-blue-700">
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            <h3 className="text-lg font-bold mb-4">{editingId ? "Modifier" : "Ajouter"} un jour férié</h3>
