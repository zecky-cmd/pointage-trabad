...
interface ProfilUtilisateur {
  id_employe: string
  employe: Employe[] // This is an array from the relationship
}
...
export default function RapportMensuelPage() {
  ...
  const loadData = async () => {
    try {
      ...
      if (profil) {
        const typedProfil = profil as ProfilUtilisateur
        const employeData =
          Array.isArray(typedProfil.employe) && typedProfil.employe.length > 0 ? typedProfil.employe[0] : null
  ...
  const loadPointagesMois = async () => {
    ...
    const dernierJourDate = `${annee}-${mois}-${String(dernierJour).padStart(2, "0")}`
        ...
        {stats && (
          <div className="grid grid-cols-4 gap-4 mb-6">
            <div className="bg-white rounded-lg shadow p-4">
              ...
              <p className="text-xs text-gray-500">{stats.joursPresent} jours travaillés</p>
            </div>
            <div className="bg-white rounded-lg shadow p-4">
            ...
            <div className="bg-white rounded-lg shadow p-4">
              ...
              <p className="text-2xl font-bold text-red-600">{stats.joursAbsent}</p>
              <p className="text-xs text-gray-500">{stats.absencesJustifiees} justifiées</p>
            </div>
            <div className="bg-white rounded-lg shadow p-4">
        ...
        <div className="bg-white rounded-lg shadow overflow-hidden">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {pointages.map((p) => (
                <tr key={p.id_pointage} className={p.statut === "weekend" ? "bg-gray-50" : ""}>
                  <td className="px-4 py-3 whitespace-nowrap text-sm">{formatDate(p.date_pointage)}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-sm font-mono">{p.pointage_arrive || "----"}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-sm font-mono">{p.pointage_pause || "----"}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-sm font-mono">{p.pointage_reprise || "----"}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-sm font-mono">{p.pointage_depart || "----"}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-sm font-semibold">
                    {formatDuree(calculerHeuresTravaillees(p))}
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap text-sm">
                    {p.retard_minutes > 0 ? (
                      <span className={p.retard_minutes > 15 ? "text-red-600 font-semibold" : "text-orange-600"}>
