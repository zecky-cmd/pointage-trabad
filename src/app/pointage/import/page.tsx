...
export default function ImportPointagesPage() {
  ...
  const genererLignesMois = async () => {
    ...
    try {
      ...
      // Appeler la fonction SQL pour générer les lignes
      await supabase.rpc('generer_lignes_employe_periode', {
  ...
  const handleSavePointage = async (pointage: any, field: string, value: string) => {
    try {
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <select
                ...
                onChange={(e) => setEmployeSelectionne(parseInt(e.target.value))}
          ...
          {employeSelectionne && employeNom && (
            <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded">
              <p className="text-sm text-blue-800">
                📋 <strong>{employeNom.prenom_employe} {employeNom.nom_employe}</strong> - {employeNom.post_employe}
              </p>
              <p className="text-xs text-blue-600 mt-1">
                Cliquez sur "Générer les lignes" pour créer toutes les lignes du mois (jours ouvrables, weekends, fériés).
                Ensuite, remplissez les heures de pointage jour par jour.
              </p>
            </div>
          )}
        </div>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {pointages.map((p) => (
                  <tr 
                    key={p.id_pointage}
                    className={
                      p.statut === 'weekend' ? 'bg-gray-50' :
                      p.statut === 'ferie' ? 'bg-yellow-50' :
                      p.statut === 'absent' ? 'bg-red-50' :
                      ''
                    }
                  >
                    ...
                    <td className="px-4 py-3 text-sm">
                      <span className={`px-2 py-1 text-xs rounded-full ${
                        ...
                        p.statut === 'ferie' ? 'bg-yellow-100 text-yellow-800' :
                        'bg-gray-100 text-gray-800'
                      }`}>
                        {p.statut === 'present' ? 'Présent' : 
                         p.statut === 'absent' ? 'Absent' :
                         p.statut === 'ferie' ? 'Férié' :
                         'Week-end'}
                      </span>
                    </td>
                  </tr>
