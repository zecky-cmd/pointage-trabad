...
export default function JustificationsPage() {
  ...
  const loadJustifications = async () => {
    ...
    if (filter === 'en_attente') {
      ...
      query = query.or('statut_justification_absence.in.(justifiee,rejetee),statut_justification_retard.in.(justifiee,rejetee)')
    }

    const { data } = await query

    // Filtrer les pointages qui ont des justifications
    let filtered = (data || []).filter(p => 
      p.justification_absence || p.justification_retard
    )
  ...
  const handleTraiter = async (id_pointage: number, type: 'retard' | 'absence', decision: 'justifiee' | 'rejetee') => {
    try {
  ...
  const getTempsEcoule = (dateStr: string) => {
    const date = new Date(dateStr)
    const now = new Date()
  ...
  const nbEnAttente = justifications.filter(j => 
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 mt-1">
                Gérer les demandes de justification des employés
              </p>
            </div>
              ...
              <div className="flex items-start justify-between">
                <div className="flex items-start space-x-4">
                  <div className="h-12 w-12 rounded-full bg-blue-500 flex items-center justify-center text-white font-bold text-lg flex-shrink-0">
                    {j.employe?.prenom_employe?.[0]}{j.employe?.nom_employe?.[0]}
                  </div>
                      ...
                      <div className="mt-4 p-4 bg-orange-50 border border-orange-200 rounded">
                        <div className="flex items-start justify-between mb-2">
                          ...
                          <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
                            j.statut_justification_retard === 'justifiee' ? 'bg-green-100 text-green-800' :
                            j.statut_justification_retard === 'rejetee' ? 'bg-red-100 text-red-800' :
                            'bg-yellow-100 text-yellow-800'
                          }`}>
                            {j.statut_justification_retard === 'justifiee' ? '✓ Approuvée' :
                             j.statut_justification_retard === 'rejetee' ? '✗ Rejetée' :
                             '⏳ En attente'}
                          </span>
                        </div>
                    ...
                    {j.justification_absence && (
                      <div className="mt-4 p-4 bg-red-50 border border-red-200 rounded">
                        <div className="flex items-start justify-between mb-2">
                          ...
                          <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
                            j.statut_justification_absence === 'justifiee' ? 'bg-green-100 text-green-800' :
                            j.statut_justification_absence === 'rejetee' ? 'bg-red-100 text-red-800' :
                            'bg-yellow-100 text-yellow-800'
                          }`}>
                            {j.statut_justification_absence === 'justifiee' ? '✓ Approuvée' :
                             j.statut_justification_absence === 'rejetee' ? '✗ Rejetée' :
                             '⏳ En attente'}
                          </span>
                        </div>
                        <p className="text-sm text-gray-700 mt-2">
